...
public class FindAllProfessorReceiver
    extends AbstractReceiver<
        FindAllProfessor.Request, Page<ProfessorResponse>> {
  ...
  @Override
  protected Page<ProfessorResponse> doExecute(
          FindAllProfessor.Request parameter) {
    ...
    if (parameter.getNome() != null && !parameter.getNome().isBlank()) {
      Page<ProfessorDocument> professorDocuments = professorRepository.findByNomeContaining(parameter.getNome(), pageRequest);
    ...
    if(parameter.getDisciplinaId() != null ) {
      Page<ProfessorDocument> professorDocuments = professorRepository.findByDisciplinaId(Integer.valueOf(parameter.getDisciplinaId()), pageRequest);
      courseResponses = mapper.map(professorDocuments.getContent());
      return new PageImpl<>(courseResponses, pageRequest, professorDocuments.getTotalElements());
    }
